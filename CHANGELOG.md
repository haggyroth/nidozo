...
- refactor(battle): **the move/switch resolution ladder was written out four times, and the doubles copies failed silently** (#291). The numeric-slot → normalised-name → fuzzy-match ladder existed in four places — `_resolve_move` and `_resolve_switch` for singles against `_match_move_in_list` and `_resolve_switch_doubles` for doubles — so a correction to one of them reached exactly one surface. **The premise reproduces this time** (unlike #287 and #290): the duplication is real and the four copies were genuinely the same ladder. All four now call one `_match_in_list(identifier, items, key, label=)`, which owns both the matching and its diagnostics; `_match_move_in_list` is deleted. **A real defect fell out of the same lines:** the doubles resolvers logged *nothing* at any level when an identifier failed to match, and the failure is not harmless there — `_parse_doubles_json` fills an unresolved slot with a pass so the other slot's action still submits, and `parse_action` returns that order directly for a `DoubleBattle` (there is no text fallback), so the top-level `No parseable action found` warning never fires either. A slot could therefore do nothing for an entire turn with no line written anywhere. Demonstrated with a real parse: the response `{"actions":[{"action_type":"move","identifier":"thunderbolt"},{"action_type":"switch","identifier":"zzzznotamon"}]}` returned a valid `DoubleBattleOrder` carrying `/choose pass` and an empty log; it now writes `doubles slot 1 switch: 'zzzznotamon' not found (have ['kingambit'])`, and an out-of-range slot number warns in doubles exactly as it always did in singles. The `label` names the candidate set at the point of failure (`move`, `switch`, `doubles slot 1 move`), which is what makes a doubles half-turn diagnosable at all. A **fifth copy is deliberately left alone**: `analysis/analyzer.py:_resolve_move_slot` parses a *recorded* `/choose` string after the battle against heuristic `move_scores` dicts — no battle object, no fuzzy step — and routing it through the shared matcher would start fuzzy-"correcting" historical actions, rewriting stats for battles already played. One correction to the audit's framing: it calls this "a maintenance hazard, not a runtime bug", and the missing diagnostics were the runtime half. **Two negative controls, and one honest limit on what the tests prove.** Restoring the file from `main` fails the two diagnostics tests with the production symptom — the doubles failure writes nothing to `caplog.text` while the singles warning still appears. Injecting a *one-sided* change (the doubles path tightening its own fuzzy cutoff to 0.97 instead of the shared one) fails the agreement tests as `assert <MagicMock id=…> is None` on `thunderolt` — the exact shape of the hazard. The three agreement tests **pass against the old code as well**: the four copies were behaviourally equivalent for matching, so they are a pin against a future one-sided fix rather than a reproduction of a matching bug, and the 18-identifier battery compares the matched *object* by identity across both paths instead of the order each path wraps around it. Suite 1431 passed, coverage 94.42%

- fix(heuristics): **the Tera defensive advisory had the type chart backwards** (#290). `_defending_mult` indexed poke-env's type chart by hand as `type_chart[move_type][tera_type]`, but the chart is keyed `{defender: {attacker: mult}}` — `type_chart[type_1.name][self.name]`, where `self` is the *attacking* type (`battle/pokemon_type.py`). Both keys were swapped, so every matchup was reported inverted: a Fire move against a Water Tera came out as "still weak to opponent's moves as WATER — no defensive benefit" when it is resisted, and a Ground move against a Flying Tera as "neutral or better" when it is **immune** — a swapped lookup misses the key and falls back to 1.0, so immunities were not merely inverted but invisible. The lookup now goes through poke-env's own `PokemonType.damage_multiplier`, which cannot be inverted, and an `== 0.0` case was added so an immunity is named rather than folded into "resists". **The audit's own premise does not reproduce.** It reads the serializer's comment "types already reflects Tera type when Terastallized (poke-env handles this)" as false and proposes emitting the effective types via `mon.type_1`/`mon.type_2`. Both halves are wrong: poke-env's `types` *is* `[type_1, type_2?]`, built from exactly those two properties, and `type_1` returns `_terastallized_type` once `_terastallized` is set — so a Terastallized Charizard reports `["STEEL"]`, not `["FIRE", "FLYING"]`, and the "remediation" would emit precisely what the serializer already emits. `types`, `tera_type` and `is_terastallized` agree; there was no contradiction to fix. The tests drive the real library — `Pokemon.terastallize()` and `start_effect("TypeChange", …)`, both public — and pin the behaviour on **both** serialization paths so a poke-env bump that changes it fails loudly rather than silently reclassifying every mon's type; the comment now says why it is true, citing the property chain, so the claim is checkable rather than asserted. One more thing fell out of the same block: the same-type-Tera test read poke-env's private `_type_1`/`_type_2`, which are the *species* types. Showdown grants 2× STAB when `getTypes(false, true).includes(type)` — the pre-Tera types with type changes included (`sim/battle-actions.ts`) — so a Grass-typed Charizard Terastallizing to Grass was being called a "new type". It now compares against `own.types`, which is the pre-Tera list while the side can still Tera (nothing on our side has Terastallized, so `type_1` is still the current type); the tests cover the type-changed case in both directions. poke-env's own `Pokemon.stab_multiplier` is **not** used for this: its predicate is `_type_1/_type_2 ∪ _temporary_types`, which keeps calling the *original* typing a same-type Tera after a type change where Showdown would not, and Showdown is the authority. The private read also defaulted to `None`, so a rename in a poke-env bump would have silently reclassified every Tera as a new type. Two negative controls: restoring the swapped lookup fails all 4 parametrized matchups; restoring the `_type_1`/`_type_2` read fails the type-change test as `assert 'same as base typing (2× STAB bonus)' in "...(new type (1.5× STAB on matching moves)..."`. Coverage rose to 94.13% on the added `== 0.0` branch

- fix(heuristics): **the speed advisory compared two different quantities, so it said "You move FIRST" nearly every turn** (#289). `_effective_speed` used the own side's *real* battle stat — EVs, IVs and nature applied, ~333 for a max-invested sweeper — and the opponent's species *base* stat, ~102. That is not a comparison, and the own side was carrying 50-100 points of investment the opponent's number could not contain, so the advisory was wrong against any speed-invested opponent while contradicting the design goal of surfacing genuine move-order nuance. Both sides now go through one `_comparable_speed`, which works from base speed with each side's **observed** stat stages and paralysis applied. The audit offered "both base stats, or both actual stats when the opponent's actual stats are revealed"; the second branch is **not reachable in this format** — poke-env fills `stats` only from `update_from_request` (the battle request, which Showdown sends to a Pokémon's owner alone) or from the teambuilder, and a Pokémon met in battle arrives through `|switch|` carrying species, level and an HP percentage, never EVs or nature — so the remediation reduces to base-for-base, which is what shipped. **The trade is explicit and stated in the code:** this answers "who is faster at equal investment", not who moves first. It gives up cases the old code got right by accident (our own max-invested mon against a faster-base opponent now reads "slower") in exchange for cases it got wrong systematically (every invested opponent). Nothing is lost from our own side — the model reads its exact stats in the serialized state — and speed ties become reachable again: with base stats they happen when two species are genuinely matched, where comparing an invested stat against a bare base almost never produced one. The advisory also stopped overclaiming: `you_move_first` is renamed **`faster_by_base_speed`** (it is set from base speed; the name asserted a move order the comparison cannot establish) and both the context note and the move-score note now say "faster/slower by base speed" instead of "You move FIRST/SECOND", as does `switch_scores[*].speed_vs_opp`. **Every speed test written before this was blind to the defect**: `_mock_pokemon` defaults `stats` to `{}`, which sends the own side down the base-stat fallback as well, so the whole suite compared base-to-base and agreed with itself — the same hand-faked-both-sides pattern this audit keeps turning up. The new tests set a real `stats` dict, which is the only shape in which the bias is visible. Two negative controls: restoring the `is_own` branch fails 5 tests with the production symptom (`assert 333 == 80`, `333.0 == 80.0` — the real stat leaking straight into the comparison); restoring the old note wording fails with `assert 'base speed' in 'You move FIRST (est. 400 vs 80)'`. Four tests pin the upstream fact instead of restating it: a Pokémon built the way the protocol builds an opponent has `stats["spe"] is None` with `base_stats["spe"] == 102`

- fix(llm): **a dead backend held each turn for twice the timeout budget** (#288). Both retry attempts ran on the full per-turn deadline, so when the first timed out the retry simply waited the whole `NIDOZO_TURN_TIMEOUT` again — 180 seconds by default, with the battle stalled and the WebSocket showing nothing — and the log said only that the turn had timed out twice. The retry now gets **half** the budget: the first attempt keeps the full deadline, because a slow-but-working local model is the ordinary case and cutting it short would break the models the deadline was sized for, while the retry is a long shot (a transient stall clearing, a local model finishing its load) and a shorter one bounds a dead backend at 1.5× the budget instead of 2×. The audit's recommended remediation — a constant or exponential **backoff sleep** between attempts — was deliberately not taken, and would have made the defect worse: the cost being complained about *is* wall-clock, and sleeping between two attempts adds to the total rather than reducing it. What bounds the turn is the deadline, not the gap between attempts. **Also fixed on the same lines:** both timeout log lines formatted the deadline with `%.0f`, which is a `TypeError` when the deadline is disabled and `_turn_timeout` is therefore `None`. `logging` catches formatting errors — it prints a logging-error traceback to stderr and drops the record — so the failure is silent and the line telling you the call timed out is the one that goes missing. The coach line is the reachable one: with the turn deadline switched off the coach is still bounded by its own backend timeout (#278), so it can time out while `_turn_timeout` is `None`. Both sites now go through `_describe_limit()`, which renders `no limit`. Tested by asserting the deadline handed to `asyncio.wait_for` for each attempt — deterministic, no reliance on scheduling — plus one test that hangs the backend for real so the wall-clock bound is measured rather than assumed. **That second test was vacuous when first written**: the stand-in backend returned an empty response immediately instead of hanging, so `wait_for` never expired and the assertion could not fail; it now sleeps long enough to be cancelled, and the negative control reports the old behaviour as `0.404s` against the `0.36s` bound. Two negative controls: restoring the full deadline for the retry fails as `[10.0, 10.0] == [10.0, 5.0]` and as `two attempts took 0.404s`; reverting the coach line to `%.0f` fails as `TypeError: must be real number, not NoneType` from inside `LogRecord.getMessage`

- fix(battle): **`hp_fraction` was an `int` for a fainted Pokémon** (#287). `round` passes an int straight through, and poke-env returns int `0` for a fainted mon, so the same field was `0` in one `state_json` row and `0.43` in the next — in every row that recorded a faint, in both the own and opponent serializations. Both sites now go through one `_hp_fraction()` helper, which coerces. **The rest of #287 does not reproduce.** The audit reads `round(current_hp_fraction, 3)` and concludes the model sees 0.1% of an opponent's HP; it does not, and neither does the serializer. Showdown reports an opponent's HP to everyone on the stream as `Math.ceil(100 * hp / maxhp)` out of 100, capped at 99 for a damaged Pokémon (`sim/pokemon.ts`, `getHealth`); the exact `hp/maxhp` goes only to that Pokémon's owner, in the battle request. poke-env stores the shared form verbatim, so an opponent's fraction is always `n/100` — a whole percent — and three decimals is an exact no-op on it. The model reads the same number the Showdown client draws its HP bar from. The proposed remediation (round the opponent to 2 decimals) is provably a no-op — for all 100 values an opponent can carry, `round(x, 3) == round(x, 2) == x` — and applying it to the shared helper would *damage* own-mon fidelity, where poke-env does parse the exact HP; a test fails if it is applied. Tested through the **real poke-env `Pokemon` and the real `set_hp_status` parser** rather than the mock helpers that assign `current_hp_fraction` directly: the claim is about what poke-env makes of the HP field Showdown sends, and a mock would agree with whatever the test asserted

- perf(db): **the model-stats page walked every row of ELO history in the database** (#286). `elo_history`'s only index was `UNIQUE(battle_id, model_id)`, and SQLite can seek a *prefix* of a composite index only — the lookup filters on `model_id`, the trailing column, so the planner had nothing to seek against. Schema v21 adds `idx_elohist_model(model_id, battle_id)`; `battle_id` is carried as the second column so the index also supplies the join key back to `battles`. The seek is flat while the scan grows with the whole table — measured with 150 history rows per model: 0.09 ms vs 0.36 ms at 6 000 rows (3.9×), 0.07 ms vs 1.29 ms at 24 000 (19×), 0.09 ms vs 4.94 ms at 96 000 (50×). End to end that is about 7% of `get_model_stats` (7.39 ms → 6.86 ms), because the per-battle history beside it dominates; the isolated ELO subquery is 4.8× faster (0.32 ms → 0.07 ms). **The index had to go in a version-increment block, not only in `_DDL_INDEXES`** — that script runs on a fresh install and on databases *behind* v4, and every deployed database is past both, so an `_DDL_INDEXES`-only change would have reached new installs and left every existing instance scanning forever. Tested with `EXPLAIN QUERY PLAN` rather than by asserting the index exists: the same query must plan as `SCAN eh` before the migration and `SEARCH eh USING INDEX idx_elohist_model` after, so the test cannot pass against an index the planner ignores. The audit also flagged the `elo_deltas` CTE in `get_tournament_standings` as a second full scan; that one is **not reproducible** — it drives from `battles` via `idx_battles_tournament` and probes `elo_history` by `battle_id`, the leading column, so its plan and timing are unchanged by any model-leading index

- fix(llm): **the default prompt version was `v1`, five versions out of date** (#285). `PromptBuilder`, `LLMPlayer`, and `BattleStore.get_or_create_model` each defaulted `prompt_version` to `"v1"` while the API sent `"v9"`, and `LLMPlayer`'s docstring still called `v6` "current". Nothing broke in production only because every call site passes an explicit version — which is the whole problem with the default: it is unreachable right up until someone constructs a player, a builder, or a model row directly, and then it silently runs the oldest prompt in the repo and records that version against the battle's rating. The identifiers now live in one dependency-free module, `llm/versions.py` (`DEFAULT_PROMPT_VERSION`, `DOUBLES_PROMPT_VERSION`, `DRAFT_PROMPT_VERSION`, and the `PromptVersion` validation literal, from which `ALL_PROMPT_VERSIONS` is derived so the two cannot disagree); the three Python defaults and the four API request defaults all point at it. The `prompt_version` column defaults are **removed** rather than bumped — `'v1'` on `models`, `'v2'` on `tournaments`, `'v6'` on `seasons` — because every insert already passes the value, so a default could only ever stamp a stale version onto a row that forgot one; without it, a forgotten value fails at insert. **The drift had already blinded 14 tests**: `PromptBuilder()` built `v1` in `tests/test_prompt_builder.py`, so a fixture written for the v1 template kept rendering v1 and every test built on it was exercising a prompt no battle runs — including one asserting the `ACTION: move` text protocol that JSON output replaced. The fixture now matches the current templates, and two tests make the drift fail loudly instead: the default must be the newest `prompts/v<N>/` with a `turn.txt.jinja`, and the fixture must render through *every* shipped version under `StrictUndefined`

- fix(llm): **every LLM prompt and response was written to the logs in full** (#284). Three sites logged the whole body at DEBUG — the rendered prompt and the raw response in `llm/openai.py`, and the response again in `battle/llm_player.py` — and the prompt is the largest thing this system produces: the complete serialized battle state, both teams' revealed information, lessons, coach advice, and every legal action, per turn, per player, twice per battle. DEBUG is also what an operator turns on to watch HTTP traffic and store timing, so the level was never really a choice about battle transcripts — and stdout plus `LOG_FILE` is exactly the artifact that gets shipped to an aggregator or pasted into an issue. The bodies now sit behind **`NIDOZO_TRACE_LLM`**, and DEBUG keeps a summary that still answers the question you had: which model, how many messages, how many characters, and how to get the rest. A traced body is capped at 20 000 characters, because the failure being traced (a runaway prompt) is the same one that would fill the disk. All three sites go through a new `llm/trace.py`; the battle-scoped `extra=` fields the player already attached are preserved on the summary line. Tested end to end through `OpenAIBackend.complete()` and `LLMPlayer.choose_move` rather than only through the helper — the defect is a call *site* logging a body directly, which unit-testing the helper cannot catch
