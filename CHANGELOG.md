...
- docs(deps): the dropped `SPECIAL_MOVES` branch is not a behaviour change for Nidozo — for `struggle` / `recharge` / `fight` the request target now wins over the entry target, but every value that can actually arrive maps to the same `"auto"` fallthrough in `_move_target_hint`, so the new and old paths agree
