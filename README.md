...
| `LOG_LEVEL` | Log verbosity (`DEBUG` / `INFO` / `WARNING` / `ERROR`) for the JSON logs. At `DEBUG`, httpx/httpcore HTTP traffic is included too. | `INFO` |
| `NIDOZO_TRACE_LLM` | Also log the **full prompt and response body** for every LLM call. Off by default: `DEBUG` above logs only a summary (model, message count, character count), because those bodies contain the complete battle state and would otherwise land in stdout and `LOG_FILE`. Set to `1` when you are debugging prompt construction. Traced bodies are capped at 20 000 characters. | `0` |
| `LOG_FILE` | Absolute path; when set, logs are written there as well as to stdout. | _(unset)_ |
...
**Authentication (#212, #276).** `/healthz` and the static web app always stay open (so health checks work and the page can load); everything else is gated when `NIDOZO_API_TOKEN` is set. In the browser, click the **🔑** button in the header and paste the token — it's kept in `sessionStorage` (gone when the tab closes, never written to disk) and attached to every request automatically (you'll also be prompted automatically on the first `401`). Programmatic clients send `Authorization: Bearer <token>` (an `X-API-Token` header also works). WebSockets carry the credential in the `Sec-WebSocket-Protocol` handshake header as `nidozo-auth.<base64url(token)>`: browsers can set that one, unlike `Authorization`, and unlike a query string it never reaches the server's access log. Non-browser clients can still use `?token=<token>`, and any value there is redacted from the logs before it is written.
